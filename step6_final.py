...
print("\n" + "="*60)
print("PHASE 6: The Final Cyclic Handshake")
print("="*60)
...
def cyclic_tx_loop(stop_event):
    cyc = 1
    # 14 (Ether) + 4 (VLAN) + 2 (FID) + 4 (Out) + 1 (IOPS) + 1 (IOCS) + 30 (Pad) + 4 (CC+DS+TS) = 60 Bytes
    padding = b"\x00" * 30 
    while not stop_event.is_set():
        cyc = (cyc + 1) % 65536
        # Data: 4B Out | IOPS: 0x80 | IOCS: 0x80
        pn_payload = struct.pack("!H", 0x8002) + b"\x00\x00\x00\x00" + b"\x80\x80" + padding + struct.pack("!H", cyc) + b"\x35\x00"
        
        # CRITICAL FIX: Injecting the 802.1Q Priority 6 VLAN Tag
...
try:
    ...
    if len(resp) > 80 and struct.unpack("<H", resp[68:70])[0] == 0:
        ...
        if valid_frames == 0:
            print("[FAIL] Still no data. Check Wireshark to see if the relay is transmitting but Windows is hiding it.")
            
    else:
        print("\n[FAIL] Handshake rejected.")

except socket.timeout:
    print("\n[FAIL] Device stopped responding. Power cycle it.")
finally:
    stop_tx.set()
    time.sleep(0.1) 
