...
print("\n" + "="*60)
print("PHASE 8: The True Master (Perfect RPC Headers)")
print("="*60)
...
# ---------------------------------------------------------
# THE FIX: A perfectly mapped DCE/RPC Header generator
# ---------------------------------------------------------
def build_rpc_header(activity_uuid, seq_num, opnum, payload_len):
    hdr = struct.pack("!BBBB 3s B", 4, 0, 0x20, 0, b'\x10\x00\x00', 0)
    hdr += uuid.UUID("dea00000-6c97-11d1-8271-006428ce90d2").bytes_le # Device UUID
    hdr += uuid.UUID("dea00001-6c97-11d1-8271-00a02442df7d").bytes_le # PNIO UUID
    hdr += activity_uuid.bytes_le                               

    # Slots: BootTime(0), IF_Ver(1), SeqNum(seq_num), OpNum(opnum)
    hdr += struct.pack("<I I I H H H H H B B", 
                       0, 1, seq_num, opnum, 0xFFFF, 0xFFFF, payload_len, 0, 0, 0)       
    return hdr
...
def build_control_req(ar_uuid, activity_uuid, command, seq):
    ...
    # OpNum 4 = Control (PrmEnd / AppReady)
...
def check_rpc_response(resp, step_name):
    ...
    if ptype == 0x02: return True, "Accepted"
        ...
        status_hex = resp[32:36].hex() if len(resp) >= 36 else "Unknown"
        return False, f"REJECTED (Hex Status: {status_hex}. The relay is still locked!)"
...
try:
    ...
    s.sendto(build_control_req(ar_uuid, act_uuid, 1, 2), (TARGET_IP, TARGET_PORT))
    resp, _ = s.recvfrom(65536)
    ...
    s.sendto(build_control_req(ar_uuid, act_uuid, 2, 3), (TARGET_IP, TARGET_PORT))
    resp, _ = s.recvfrom(65536)
    success, msg = check_rpc_response(resp, "ApplicationReady")
    if not success: raise Exception(f"AppReady Failed: {msg}")
    print(f"   -> {msg}")
        
    print("\n[SUCCESS] Starting VLAN-Tagged Master RUN phase...")
    ...
    if valid_frames == 0:
        print("[FAIL] Handshake perfect, but still no data.")
    ...
    # OpNum 1 = True Release
    rel_hdr = build_rpc_header(act_uuid, 4, 1, 0)
