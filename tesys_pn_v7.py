#!/usr/bin/env python3
"""
PROFINET IO Controller — TeSys Tera Motor Management Relay  [v7]
===========================================================
...
def list_interfaces() -> None:
    ...
    Call from a Python shell:
      python -c "from tesys_pn_v7 import list_interfaces; list_interfaces()"
    """
    try:
...
class PNIOController:
    ...
    def read_cyclic_data(self, duration_s: float = CYCLIC_DURATION_S):
        ...
        bpf = "ether proto 0x8892 or (vlan and ether proto 0x8892)"
        ...
        try:
            ...
            for frm in (frames.results or []):
                if frm.haslayer(Ether):
                    if frm[Ether].src.lower() == CONTROLLER_MAC.lower():
                        continue
                raw = bytes(frm)
        ...
        if count == 0:
            ...
            log.warning("  3. Our output frames (FrameID=0x%04x) reach the device",
                        self.rt_frame_id_out)
            frames_list = frames.results or []
            if frames_list:
                ...
                for frm in frames_list:
                    try:
                        if not frm.haslayer(Ether):
                            continue
                        src = frm[Ether].src.lower()
                        if src == CONTROLLER_MAC.lower():
                            continue
                        if frm.haslayer(Dot1Q) and frm[Dot1Q].type == PROFINET_ETYPE and frm.haslayer(Raw):
                            payload = bytes(frm[Raw])
                        elif frm.haslayer(Ether) and frm[Ether].type == PROFINET_ETYPE and frm.haslayer(Raw):
                            payload = bytes(frm[Raw])
                        else:
                            continue
                        if len(payload) < 2:
                            continue
                        fid = struct.unpack_from(">H", payload, 0)[0]
                        fid_counts[fid] += 1
                        src_counts[src] += 1
                    except Exception:
                        continue
                if fid_counts:
                    top = ", ".join(f"0x{k:04x}({v})" for k, v in fid_counts.most_common(6))
                    log.warning("  Captured %d PROFINET frame(s) from target; FrameIDs seen: %s",
                                len(frames_list), top)
                    src_top = ", ".join(f"{k}({v})" for k, v in src_counts.most_common(3))
                    if src_top:
                        log.warning("  Source MACs seen: %s", src_top)
                else:
                    log.warning("  Captured %d frame(s) but no PROFINET payloads decoded",
                                len(frames_list))
    ...
    def run(self):
        ...
        if epm_uuid is not None:
            profiles += [
                ("epm-primary", epm_uuid, 0x0001, 0x0001, DEFAULT_WIRE_PROFILE),
                ("epm-legacy",  epm_uuid, 0x8000, 0x0100, DEFAULT_WIRE_PROFILE),
            ]
        profiles += [
            ("primary", PNIO_CM_OBJ_UUID, 0x0001, 0x0001, DEFAULT_WIRE_PROFILE),
            ("legacy",  PNIO_CM_OBJ_UUID, 0x8000, 0x0100, DEFAULT_WIRE_PROFILE),
            ("mixed",   PNIO_CM_OBJ_UUID, 0x8000, 0x0001, DEFAULT_WIRE_PROFILE),
            ("vendor",  vendor_uuid,      0x8000, 0x0100, DEFAULT_WIRE_PROFILE),
            ("mac",     mac_uuid,         0x8000, 0x0100, DEFAULT_WIRE_PROFILE),
            ("mac-mix", mac_uuid,         0x0001, 0x0001, DEFAULT_WIRE_PROFILE),
        ]
