...
# 288-byte IODWriteReq stub — byte-exact match to Tanmay 1 (1).pcapng pkt#314.
# Contains two IODWriteReqHeader blocks followed by 140 bytes of MRP config data.
# AR UUID is patched at offsets 28:44 and 92:108 before sending.
_IOD_WRITE_MRP_STUB_TEMPLATE = bytes.fromhex(
    "0c0100000c0100000c010000000000000c010000"
    "0008003c01000000"
    "00000000000000000000000000000000"  # ARUUID placeholder @ stub[28:44]
    "ffffffffffffffff0000e040000000cc"
    "000000000000000000000000000000000000000000000000"
    "0008003c01000001"
    "00000000000000000000000000000000"  # ARUUID placeholder @ stub[92:108]
...
def build_connect_stub(ar_uuid: uuid.UUID,
    ...
    if wire_profile.add_connect_prefix:
        blen = len(stub)
        # NDR prefix matched byte-for-byte against working PROFINET Commander pcap:
        #   val[0] = blen + 20  (total stub len = NDR-prefix + blocks)
        #   val[1] = 0
        #   val[2] = blen + 0x2A  (max-count / alloc hint with 42-byte offset)
        #   val[3] = 0
        #   val[4] = blen         (actual block bytes)
        prefix = struct.pack("<IIIII", blen + 20, 0, blen + 0x2A, 0, blen)
        stub = prefix + stub
    return stub
...
def pnio_spoof_connect(timeout: float = 8.0) -> bool:
    ...
    def _try_spoof_mac(spoof_mac: str, spoof_ip: str, spoof_sport: int,
        ...
        """
        Station-restart spoof: Connect then IMMEDIATELY Release (no PrmEnd).

        WHY NO PrmEnd:
          PROFINET §4.4.1 state machine:
            IDLE → [ConnectReq] → WAIT_FOR_PARAMETER
              AlarmCR: allocated in RAM only.  NVM: NOT YET written.
            WAIT_FOR_PARAMETER → [PrmEnd] → PARAMETER_END
              NVM write: happens HERE (config + AR record committed to NVM).
              AlarmCR flag: written to NVM at this point.
            WAIT_FOR_PARAMETER → [Release] → IDLE
              NVM write: NONE.  AlarmCR: freed from RAM.  No new ghost AR.

          The previous approach (Connect → PrmEnd → AppReady → Release) caused
          the device to write a NEW ghost AR to NVM at PrmEnd, which then
          survived the Release — creating a permanent new lock instead of
          clearing the old one.

          By sending Release BEFORE PrmEnd, the device returns to IDLE without
          ever committing anything to NVM.  The station-restart (§4.4.1.6) at
          the Connect step has already overwritten the old ghost AR UUID in NVM
          with the fresh UUID — but since we Release before PrmEnd, the device
          never commits the AlarmCR flag, so the NVM ends up with a stale entry
          that has no AlarmCR lock bit set.  Our normal Connect (same MAC, yet
          another new UUID) triggers another station-restart that clears it
          completely.
        """
        fresh_ar_uuid  = uuid.uuid4()
        fresh_sess_key = 1
        log.info("  [%s] MAC=%s  fresh_ar=%s  strategy=Connect+Release(no PrmEnd)",
                 label, spoof_mac, fresh_ar_uuid)

        act_uuid = uuid.uuid1(node=int(spoof_mac.replace(":", ""), 16))

        # Build ConnectReq with alarm_cr=True so the §4.4.1.6 station-restart
        # fully replaces the old ghost AR entry (including its AlarmCR slot).
        ...
        for dport in (PNIO_AR_SESSION_PORT, PNIO_UDP_PORT):
            log.info("  [%s] ConnectReq → port %d ...", label, dport)
            xport = ScapyTransport(
                iface=SCAPY_IFACE,
            ...
            try:
                conn_resp = xport.send_recv(conn_pkt,
                                            f"SpoofConnect-{label}-{dport}", timeout)
                if conn_resp is None:
                    log.warning("    [%s] No response on port %d (timeout)", label, dport)
                    continue

                rpc_ptype = conn_resp[1] if len(conn_resp) > 1 else 0xFF
                sb = conn_resp[80:] if len(conn_resp) > 80 else b""

                if rpc_ptype == 0x06:
                    log.warning("  [%s] DCE/RPC REJECT on port %d", label, dport)
                    continue
                if rpc_ptype != 0x02:
                    log.warning("  [%s] ptype=0x%02x (unexpected) on port %d",
                                label, rpc_ptype, dport)
                    continue

                # ── Discriminate ConnectRes success vs error ─────────────────
                # SUCCESS: IODConnectRes block type 0x0112 at stub offset 20
                #          OR stub[0] byte (LE NDR ArgsMax LSB) != 0x01
                # ERROR:   stub[2] == 0x81 (Code1=AlarmCR locked) in any layout
                #          BOTH old format (ErrorCode=0x01) and new (ErrorCode=0x00)
                #          have byte[2]=0x81 when AlarmCR is locked.
                is_success = False
                if len(sb) >= 22:
                    blk_type = struct.unpack_from(">H", sb, 20)[0]
                    if blk_type == 0x0112:
                        is_success = True
                if not is_success and len(sb) >= 4:
                    # Unified error check: Code1=0x81 is always at byte[2]
                    # regardless of whether ErrorCode is 0x01 or 0x00
                    if sb[2] == 0x81:
                        err32 = struct.unpack_from(">I", sb, 0)[0]
                        log.warning("  [%s] AlarmCR locked (0x%08x) on port %d — "
                                    "station-restart not effective here", label, err32, dport)
                        continue
                    # Non-error NDR prefix → success
                    if sb[0] not in (0x01, 0x00) or (sb[0] == 0x00 and sb[2] == 0x00):
                        is_success = True

                if not is_success:
                    log.warning("  [%s] Ambiguous ConnectRes stub on port %d: %s",
                                label, dport, sb[:24].hex())
                    # Treat as success if length looks right (device accepted but
                    # our discriminator missed the block type)
                    if len(sb) >= 20 and sb[2] != 0x81:
                        log.warning("  [%s] Treating as success (no 0x81 in byte[2])", label)
                        is_success = True
                    else:
                        continue

                log.info("  [%s] ConnectRes ACCEPTED on port %d  "
                         "(device now in WAIT_FOR_PARAMETER)", label, dport)

                # ── Release immediately — DO NOT send PrmEnd ─────────────────
                # Device is in WAIT_FOR_PARAMETER.  NVM has NOT been written yet.
                # Sending Release here returns device to IDLE without NVM write.
                log.info("  [%s] Sending Release immediately (before PrmEnd) "
                         "to avoid NVM write ...", label)
                rel_resp = xport.send_recv(rel_pkt,
                                           f"SpoofRelease-{label}-{dport}", 4.0)
                if rel_resp is not None:
                    rr_ptype = rel_resp[1] if len(rel_resp) > 1 else 0xFF
                    if rr_ptype == 0x02:
                        log.info("  [%s] Release accepted — device back to IDLE, "
                                 "NVM NOT written, ghost AR cleared.", label)
                    else:
                        log.warning("  [%s] Release response ptype=0x%02x "
                                    "(expected 0x02)", label, rr_ptype)
                else:
                    log.warning("  [%s] Release timed out — "
                                "device may still have the AR in WAIT_FOR_PARAMETER", label)

                # Brief pause for device state machine to settle
                time.sleep(1.5)
                log.info("  [%s] Ghost AR purge complete.", label)
                return True
    ...
    # ── Attempt 1: Spoof our OWN controller MAC ────────────────────────────
    # The ghost AR was most likely created by CONTROLLER_MAC crashing without Release.
    log.info("  Attempt 1/2: Spoof OWN controller MAC=%s (ghost AR owner)", CONTROLLER_MAC)
    ...
    # ── Attempt 2: Spoof the Tanmay demo PLC MAC ──────────────────────────
    # In case the ghost AR was from the Tanmay demo PLC (60:7d:09:5b:24:b6).
    log.info("  Attempt 2/2: Spoof demo PLC MAC=%s (Tanmay session)", DEMO_SESSION_MAC)
    if _try_spoof_mac(DEMO_SESSION_MAC, DEMO_SESSION_IP, DEMO_SESSION_SPORT,
                      "pnc1", "demo-mac"):
        return True

    log.error("  All spoof-connect attempts failed.")
    log.error("  The ghost AR is in device NVM and cannot be cleared via software.")
...
class PNIOController:
    ...
    def step_ar_connect(self, include_alarm_cr: bool = True) -> bool:
        ...
        # PROFINET ConnectRes stubs come in three observed flavours on TeSys Tera:
        #
        #   SUCCESS (large IODConnectRes):
        #     stub[0:4]  LE = ArgsMax (e.g. 0x000015a0)  → stub[0] is typically 0xa0
        #     stub[20:22] BE = 0x0112 (IODConnectRes block type)
        #     len(stub) >> 30 bytes
        #
        #   ERROR old format (pre-NDR-fix):
        #     stub[0:4] BE = 0x010181db  (ErrorCode=0x01, Decode=0x01,
        #                                  Code1=0x81=AlarmCR, Code2=0xdb)
        #     stub[0]==0x01
        #
        #   ERROR new format (post-NDR-fix, this firmware path):
        #     stub[0:4] BE = 0x004081db  (ErrorCode=0x00, Decode=0x40,
        #                                  Code1=0x81=AlarmCR, Code2=0xdb)
        #     stub[0]==0x00,  stub[2]==0x81
        #
        # UNIVERSAL DISCRIMINATOR:
        #   byte[2] == 0x81 AND len(stub) < 30  →  AlarmCR ghost-AR lock error
        #   block_type at stub[20:22] == 0x0112  →  success
        #   Everything else with len >= 30        →  treat as success
        is_error   = False
        err_status = 0

        if len(stub_body) < 30:
            # Short stub — must be an error response
            ...
            if len(stub_body) >= 3 and stub_body[2] == 0x81:
                ...
                log.error("    ConnectRes PNIO error: 0x%08x "
                          "(ErrorCode=0x%02x  Decode=0x%02x  Code1=0x81[AlarmCR locked]"
                          "  Code2=0x%02x)",
                          err_status,
                          (err_status >> 24) & 0xFF,
                          (err_status >> 16) & 0xFF,
                ...
                log.error("    ConnectRes PNIO error: 0x%08x  Code1=0x%02x[%s]",
                          err_status, ec1, EC1_NAMES.get(ec1, "unknown"))
                if ec1 == 0x81:
                    self._ghost_ar_detected = True
            return False

        # Long stub — check for IODConnectRes block type at offset 20
        if len(stub_body) >= 22:
            blk_type = struct.unpack_from(">H", stub_body, 20)[0]
            if blk_type == 0x0112:
                pass  # confirmed success — fall through to OK path
            elif stub_body[2] == 0x81:
                # Error block embedded in a longer stub
                err_status = struct.unpack_from(">I", stub_body, 0)[0]
                log.error("    ConnectRes PNIO ghost-AR error: 0x%08x", err_status)
                self._ghost_ar_detected = True
                return False

        # ── IODConnectRes: extract echoed IOD data length ──────────────────
        # In a SUCCESS response the NDR prefix[1] (LE) = ArgsLen (bytes of blocks).
        # Do not read stub[4:8] as an error; it is part of the NDR MaxCount.
        iod_len = struct.unpack_from("<I", stub_body, 0)[0] if len(stub_body) >= 4 else 0
        if iod_len == 0 and len(stub_body) < 30:
            log.error("    ConnectRes: response too short — device silently rejected connect")
            log.debug("    Full stub: %s", stub_body.hex())
            return False

        log.info("    ConnectRes OK  (%d bytes, NDR_ArgsMax=%d)", len(resp), iod_len)
        return True

    def step_iod_write_mrp(self) -> bool:
        """
        IODWriteReq (opnum=3, seq=1) — 2-record MRP configuration write.

        Byte-exact match to Tanmay 1 (1).pcapng packet #314 (index 313).
        The stub is a constant 288-byte template; only the AR UUID is patched
        in at offsets 28 and 92 before sending.

        Correct sequence (Tanmay-confirmed):
          seq=0  ConnectReq  → ConnectRes
          seq=1  IODWriteReq (this method)
          seq=2  PrmEnd
          seq=3  AppReady
        """
        log.info("-- IODWrite MRP (opnum=%d, seq=%d) --", OP_WRITE, self.seq_num)
        stub = bytearray(_IOD_WRITE_MRP_STUB_TEMPLATE)
        ar_bytes = _u(self.ar_uuid)
        stub[28:44] = ar_bytes
        stub[92:108] = ar_bytes
        resp = self._send(OP_WRITE, bytes(stub), "IODWriteMRP", timeout=5.0)
        if resp is None:
            return False
        log.info("    IODWriteRes OK")
        return True
    ...
    def run(self):
        ...
        for attempt, (pname, obj_uuid, subslot, sess_key, wprof, alarm_cr) in enumerate(profiles, start=1):
            ...
            if self._ghost_ar_detected:
                ...
                # ── STEP -1: Spoofed Release with original PLC session params ──
                # Send a Release using the exact DCE/RPC session parameters from
                # the original Siemens PLC demo session (activity UUID, AR UUID,
                # session key, MAC).  This is the most targeted method to clear
                # the NVM ghost AR without a power cycle.
                log.info("Step -1: Spoofed Release (exact PLC session params) ...")
                spoof_rel_ok = pnio_spoof_release(timeout=5.0)
                if spoof_rel_ok:
                    log.info("  Spoofed Release accepted — waiting 2 s ...")
                    time.sleep(2.0)
                    if _retry_connect_all_profiles("spoof-release"):
                        pass  # success — fall through to PrmEnd / AppReady / cyclic
                    # If still failing, fall through to Step 0
                else:
                    log.warning("  Spoofed Release did not clear ghost AR — trying station-restart ...")

                # ── STEP 0: Station-restart with our OWN MAC (§4.4.1.6) ───────
                # Strategy: Connect→Release(no PrmEnd) twice with fresh UUIDs.
                #
                # Round 1 clears the old ghost AR from NVM via station-restart
                # and leaves the device in IDLE without writing a new NVM entry
                # (because we Release before PrmEnd).
                #
                # Round 2: the same MAC reconnects again with yet another new UUID.
                # This second station-restart overwrites whatever transient NVM
                # state Round 1 left (if any) and confirms the device is truly
                # ready to accept our real Connect.
                #
                # Then the real Connect (same MAC, a third new UUID) works because
                # the device sees the same MAC and does a final station-restart
                # that completes normally.
                log.info("Step 0: Auto station-restart ×2 with own MAC (§4.4.1.6) ...")
                spoof_round1 = pnio_spoof_connect(timeout=8.0)
                if spoof_round1:
                    log.info("  Round 1 station-restart OK. Waiting 2 s ...")
                    time.sleep(2.0)
                    # Round 2: a second fresh station-restart clears any residual NVM state
                    log.info("  Round 2 station-restart (clearing residual NVM state) ...")
                    spoof_round2 = pnio_spoof_connect(timeout=8.0)
                    if spoof_round2:
                        log.info("  Round 2 OK. Waiting 2 s then connecting ...")
                        time.sleep(2.0)
                    else:
                        log.warning("  Round 2 spoof failed (may still work). Trying connect ...")
                        time.sleep(1.0)

                    if _retry_connect_all_profiles("station-restart ×2"):
                        pass  # success — fall through to PrmEnd / AppReady / cyclic
                    else:
                        log.error("Connect still failing after station-restart ×2.")
        ...
        # seq=1  IODWrite MRP configuration (byte-exact match to Tanmay pkt#314)
        if not self.step_iod_write_mrp():
            log.error("IODWrite MRP failed — aborting")
