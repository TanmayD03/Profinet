...
# IODWriteReq MRP config stub template (288 bytes, confirmed from Tanmay pkt#314).
# Two IODWriteReqHeader blocks (64 bytes each) + 140 bytes constant MRP RecordData.
# AR UUID (bytes_le) is substituted at runtime at stub offsets 28 and 92.
_IOD_WRITE_MRP_STUB_TEMPLATE = bytes.fromhex(
    # NDR prefix: ArgsMax=ArgsLen=MaxCount=268, Offset=0, ActualCount=268
    "0c0100000c0100000c010000000000000c010000"
    # Block 1: IODWriteReqHeader — WriteMultiple (Index=0xe040), SeqNum=0
    # BlockType(2)=0x0008, BlockLen(2)=60, Ver=1.0, SeqNum(2)=0
    "0008003c01000000"
    # ARUUID placeholder (16 bytes, bytes_le) — replaced at runtime
    "00000000000000000000000000000000"
    # API=0xffffffff, Slot=0xffff, SubSlot=0xffff, Pad=0, Index=0xe040, RDL=204
    "ffffffffffffffff0000e040000000cc"
    # RWErrorInfo + padding (24 bytes)
    "000000000000000000000000000000000000000000000000"
    # Block 2: IODWriteReqHeader — PDPortDataRealCheck (Index=0x80b0), SeqNum=1
    "0008003c01000001"
    # ARUUID placeholder (16 bytes, bytes_le) — replaced at runtime
    "00000000000000000000000000000000"
    # API=0, Slot=0, SubSlot=0x8000, Pad=0, Index=0x80b0, RDL=140
    "0000000000008000000080b00000008c"
    # RWErrorInfo + padding (24 bytes)
    "000000000000000000000000000000000000000000000000"
    # RecordData (140 bytes): MRP domain config — fully constant
    "0401002c010000000000000000008000000080510213001801000000"
    "c3d687fe789e03a1acdbe5bfcbbc27b600000000"
...
COMMANDER_WIRE_PROFILE = WireProfile(
    ...
    rt_send_tagged=True,
    rt_send_untagged=False,   # Tanmay pcap: only VLAN-tagged output frames
)
...
def build_connect_stub(ar_uuid: uuid.UUID,
    ...
    if wire_profile.add_connect_prefix:
        blen = len(stub)
        # NDR prefix verified byte-for-byte against Tanmay working pcap (pkt 304):
        #   val[0] = blen + 0x2A  (ArgsMax / alloc-hint)
        #   val[1] = blen         (MaxCount)
        #   val[2] = blen + 0x2A  (Offset / alloc-hint repeated)
        #   val[3] = 0
        #   val[4] = blen         (ActualCount)
...
def build_release_stub(ar_uuid: uuid.UUID,
                       session_key: int = DEFAULT_SESSION_KEY) -> bytes:
    """
    Build an IODReleaseReq NDR stub (opnum 1).

    Block type 0x0114 (IODControlReq with ControlCommand=Release).
    NDR prefix uses standard {blen, blen, blen, 0, blen} form.
    Called by pnio_spoof_connect() for the cleanup Release after a
    spoofed Connect session, and usable standalone for targeted releases.
    """
    body  = struct.pack(">H", 0x0000)          # Padding
    body += _u(ar_uuid)                         # ARUUID
    body += struct.pack(">H", session_key)      # SessionKey
    body += struct.pack(">H", 0x0000)          # Padding
    body += struct.pack(">HH", 0x0004, 0x0000) # ControlCommand=Release + reserved
    block = _block(0x0114, body)
    blen  = len(block)
    ndr_prefix = struct.pack("<IIIII", blen, blen, blen, 0, blen)
    return ndr_prefix + block
...
class ScapyTransport:
    ...
    def __init__(self, iface: str,
        ...
        self.sport   = sport
        self.dport   = dport
        self.last_resp_sport: int = 0   # set by send_recv from response UDP sport
        ...
        def _handler(pkt):
            ...
            if pkt.haslayer(Raw):
                ...
                if resp_q.empty():
                    if pkt.haslayer(UDP):
                        try:
                            self.last_resp_sport = int(pkt[UDP].sport)
                        except Exception:
                            pass
                    resp_q.put(raw)
...
def dcp_set_station_name(iface: str, src_mac: str, target_mac: str,
    ...
    dcp_req += struct.pack(">I", xid)
    dcp_req += struct.pack(">H", 0)
    dcp_req += struct.pack(">H", len(dcp_blk))   # DataLength = block header (4) + name + padding
    dcp_req += dcp_blk
...
def pnio_spoof_release(timeout: float = 5.0) -> bool:
    ...
    for dport in (PNIO_AR_SESSION_PORT, PNIO_UDP_PORT):
        ...
        frame = (Ether(dst=TARGET_MAC, src=DEMO_SESSION_MAC) /
                 IP(src=DEMO_SESSION_IP, dst=TARGET_IP) /
...
def pnio_spoof_connect(timeout: float = 8.0) -> bool:
    """
    Send a PROFINET ConnectReq that triggers IEC 61158-6-10 §4.4.1.6 Station Restart.

    The ghost AR was almost certainly created by OUR OWN CONTROLLER_MAC
    (18:3d:2d:61:f9:70) in a previous run that crashed before sending Release.
    Per §4.4.1.6, when the SAME CMInitiatorMACAddress reconnects with a
    DIFFERENT ARUUID, the device MUST abort the stale AR and accept the new one.

    Strategy (tries both our own MAC and the Tanmay demo MAC):
      1. Build a fresh ConnectReq (new ARUUID) from CONTROLLER_MAC
      2. Send to PNIO_AR_SESSION_PORT (49152) and PNIO_UDP_PORT (34964)
      3. If accepted: send PrmEnd → AppReady → Release to fully clear ghost AR
      4. Fall back to DEMO_SESSION_MAC if our own MAC doesn't work

    Returns True if the ghost AR was cleared.
    """
    if not SCAPY_OK:
    ...
    def _try_spoof_mac(spoof_mac: str, spoof_ip: str, spoof_sport: int,
                       station_name: str, label: str) -> bool:
        """Attempt station-restart spoof with a given source MAC."""
        fresh_ar_uuid  = uuid.uuid4()
        fresh_sess_key = 1
        log.info("  [%s] Spoofing MAC=%s  fresh_ar_uuid=%s", label, spoof_mac, fresh_ar_uuid)

        stub = build_connect_stub(
            fresh_ar_uuid,
            ctrl_mac=spoof_mac,
            controller_station_name=station_name,
            in_len=INPUT_LEN,
            out_len=OUTPUT_LEN,
        ...
        # Build activity UUID with spoofed MAC embedded (standard PROFINET format)
        mac_b    = bytes(int(x, 16) for x in spoof_mac.split(":"))
        act_uuid = uuid.uuid1(node=int(spoof_mac.replace(":", ""), 16))

        pkt = build_request(
            seq_num=0, opnum=OP_CONNECT,
            obj_uuid=PNIO_CMD_OBJ_UUID,
            if_uuid=PNIO_CM_IF_UUID,
            act_uuid=act_uuid,
            stub=stub,
        ...
        for dport in (PNIO_AR_SESSION_PORT, PNIO_UDP_PORT):
            log.info("  [%s] Sending ConnectReq to port %d ...", label, dport)
            xport = ScapyTransport(
                iface=SCAPY_IFACE,
                src_mac=spoof_mac,   src_ip=spoof_ip,
                dst_mac=TARGET_MAC,  dst_ip=TARGET_IP,
                sport=spoof_sport,   dport=dport,
                dummy_sock_ip=CONTROLLER_IP,
            )
            resp = xport.send_recv(pkt, f"SpoofConnect-{label}-{dport}", timeout)
            xport.close()

            if resp is None:
                log.warning("    [%s] No response on port %d (timeout)", label, dport)
                continue
            ...
            if rpc_ptype == 0x06:
                log.warning("  [%s] DCE/RPC REJECT on port %d", label, dport)
                continue

            if rpc_ptype != 0x02:
                log.warning("  [%s] Unexpected ptype=0x%02x on port %d", label, rpc_ptype, dport)
                continue

            # Detect success vs error using the correct discriminator:
            # SUCCESS: stub[0]==0x00 (LE ArgsMax LSB) OR block_type at stub[20:22]==0x0112
            # ERROR:   stub[0]==0x01 (PNIO ErrorCode) AND stub[2]!=0x00 (Code1)
            is_success = False
            ghost_ar_in_spoof = False

            # Apply the same 4-rule ConnectRes check as step_ar_connect:
            # Rule 1 — classic PNIO error (byte[0]==0x01, Code1 non-zero)
            if len(sb) >= 4 and sb[0] == 0x01 and sb[2] != 0x00:
                err = struct.unpack_from(">I", sb, 0)[0]
                ec1 = (err >> 8) & 0xFF
                log.warning("  [%s] PNIO error 0x%08x (Code1=0x%02x) on port %d",
                            label, err, ec1, dport)
                if ec1 == 0x81:
                    ghost_ar_in_spoof = True
                continue

            # Rule 2 — stub too short to contain any IOCRBlockRes
            if len(sb) < 30:
                ec1 = sb[2] if len(sb) >= 3 else 0
                if ec1 == 0x81:
                    ghost_ar_in_spoof = True
                    log.warning("  [%s] ConnectRes: short stub (ghost AR error 0x%02x%02x%02x%02x)"
                                " — station-restart did NOT clear device AR",
                                label, sb[0], sb[1], sb[2], sb[3] if len(sb)>3 else 0)
                else:
                    log.warning("  [%s] ConnectRes: stub too short (%d bytes) on port %d",
                                label, len(sb), dport)
                continue

            # Rule 3 — NDR ActualCount == 0 (catches 004081db with byte[2]=0x81)
            if len(sb) >= 20:
                actual_count = struct.unpack_from("<I", sb, 16)[0]
                if actual_count == 0:
                    ec1 = sb[2] if len(sb) >= 3 else 0
                    if ec1 == 0x81:
                        ghost_ar_in_spoof = True
                        log.warning("  [%s] ConnectRes NDR ActualCount=0 (0x%02x%02x%02x%02x)"
                                    " — device STILL locked, station-restart failed",
                                    label, sb[0], sb[1], sb[2], sb[3])
                    else:
                        log.warning("  [%s] ConnectRes NDR ActualCount=0 on port %d — rejected",
                                    label, dport)
                    continue

            # Rule 4 — confirm success by IODConnectRes block type 0x0112 at offset 20
            if len(sb) >= 22:
                blk_type = struct.unpack_from(">H", sb, 20)[0]
                if blk_type == 0x0112:
                    is_success = True
                else:
                    log.warning("  [%s] ConnectRes: unexpected block type 0x%04x at offset 20",
                                label, blk_type)
                    continue
            else:
                log.warning("  [%s] ConnectRes: stub too short to read block type", label)
                continue

            log.info("  [%s] ConnectRes ACCEPTED on port %d! Ghost AR cleared.", label, dport)

            # ── Complete the AR: IODWrite → PrmEnd → AppReady → Release ──────
            ...
            try:
                # seq=1: IODWrite MRP config (mandatory between ConnectRes and PrmEnd)
                iod_stub = bytearray(_IOD_WRITE_MRP_STUB_TEMPLATE)
                _ar_le = _u(fresh_ar_uuid)
                iod_stub[28:44] = _ar_le
                iod_stub[92:108] = _ar_le
                iod_pkt = build_request(seq_num=1, opnum=OP_WRITE,
                                        obj_uuid=PNIO_CMD_OBJ_UUID, if_uuid=PNIO_CM_IF_UUID,
                                        act_uuid=act_uuid, stub=bytes(iod_stub),
                                        wire_profile=COMMANDER_WIRE_PROFILE)
                iod_resp = xport2.send_recv(iod_pkt, "SpoofIODWrite-MRP", timeout)
                if iod_resp is not None:
                    log.info("  [%s] IODWrite-MRP accepted.", label)
                else:
                    log.warning("  [%s] IODWrite-MRP no response (continuing to PrmEnd).", label)

                prm_block = build_control_stub(fresh_ar_uuid, CTRL_CMD_PRM_END,
                                               session_key=fresh_sess_key)
                _blen = len(prm_block)
                prm_stub = struct.pack("<IIIII", _blen, _blen, _blen, 0, _blen) + prm_block
                prm_pkt  = build_request(seq_num=2, opnum=OP_CONTROL,
                                         obj_uuid=PNIO_CMD_OBJ_UUID, if_uuid=PNIO_CM_IF_UUID,
                                         act_uuid=act_uuid, stub=prm_stub,
                                         wire_profile=COMMANDER_WIRE_PROFILE)
                prm_resp = xport2.send_recv(prm_pkt, "SpoofPrmEnd", timeout)
                if prm_resp is not None:
                    log.info("  [%s] PrmEnd accepted — sending AppReady...", label)
                    app_block = build_control_stub(fresh_ar_uuid, CTRL_CMD_APP_READY,
                                                   session_key=fresh_sess_key)
                    _blen2 = len(app_block)
                    app_stub = struct.pack("<IIIII", _blen2, _blen2, _blen2, 0, _blen2) + app_block
                    app_pkt  = build_request(seq_num=3, opnum=OP_CONTROL,
                                             obj_uuid=PNIO_CMD_OBJ_UUID, if_uuid=PNIO_CM_IF_UUID,
                                             act_uuid=act_uuid, stub=app_stub,
                                             wire_profile=COMMANDER_WIRE_PROFILE)
                    app_resp = xport2.send_recv(app_pkt, "SpoofAppReady", timeout)
                    if app_resp is not None:
                        log.info("  [%s] AppReady accepted.", label)

                log.info("  [%s] Sending Release to clean up spoofed AR...", label)
                rel_stub = build_release_stub(fresh_ar_uuid, session_key=fresh_sess_key)
                rel_pkt  = build_request(seq_num=4, opnum=OP_RELEASE,
                                         obj_uuid=PNIO_CMD_OBJ_UUID, if_uuid=PNIO_CM_IF_UUID,
                                         act_uuid=act_uuid, stub=rel_stub,
                                         wire_profile=COMMANDER_WIRE_PROFILE)
                rel_resp = xport2.send_recv(rel_pkt, "SpoofRelease", 4.0)
                if rel_resp is not None:
                    log.info("  [%s] Release sent — ghost AR fully cleaned up.", label)
                else:
                    log.warning("  [%s] Release timed out (ghost AR may linger briefly).", label)
            finally:
                xport2.close()

            log.info("  [%s] Ghost AR cleared! Re-run without --spoof-connect to connect.", label)
            return True

        return False

    # ── Only spoof OWN controller MAC ──────────────────────────────────────
    # The ghost AR was created by THIS controller (CONTROLLER_MAC) in a
    # previous crashed/killed session.  Per §4.4.1.6 the device MUST abort
    # the old AR when the same CMInitiatorMACAddress reconnects with a new AR UUID.
    # The Siemens/Tanmay demo MAC is irrelevant to this setup and is NOT tried here.
    log.info("  Spoofing OWN controller MAC=%s (§4.4.1.6 station-restart)", CONTROLLER_MAC)
    own_sport = random.randint(49152, 65535)
    if _try_spoof_mac(CONTROLLER_MAC, CONTROLLER_IP, own_sport,
                      CONTROLLER_STATION_NAME, "own-mac"):
        return True

    log.error("  Station-restart spoof failed.")
    log.error("  The device keeps returning 0x004081db (ghost AR locked) even to our own MAC.")
    log.error("  This device requires a POWER CYCLE to clear the NVM ghost AR.")
    log.error("  ┌─────────────────────────────────────────────────────────────────┐")
    log.error("  │  ACTION REQUIRED:                                               │")
    log.error("  │  1. Unplug TeSys Tera power cable                               │")
    log.error("  │  2. Wait 60+ seconds (NVM writeback timeout)                    │")
    log.error("  │  3. Reconnect power and wait 15 s for boot                      │")
    log.error("  │  4. Rerun this script                                            │")
    log.error("  │  OR: Use SoMove via USB service cable → Reset Communication     │")
    log.error("  └─────────────────────────────────────────────────────────────────┘")
    log.error("  Required actions (in order):")
    log.error("    1. Power-cycle TeSys Tera for 60+ seconds (not just restart)")
    log.error("    2. After power-on wait 15 s, then rerun with --wait-hello")
    log.error("    3. Use SoMove via USB service cable -> 'Reset Communication'")
    return False


def extract_ghost_ar_uuid() -> list:
    """
...
class PNIOController:
    ...
    def __init__(self):
        ...
        # Bind extra dummy socket on CMD_CM_UDP_PORT (0x8892 = 34962) so that
        # when the device sends its AppReady callback to controller:34962,
        # Windows does NOT send ICMP "Port Unreachable" which would abort the AR.
        self._cm_sock: Optional[socket.socket] = None
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
            s.bind((CONTROLLER_IP, CMD_CM_UDP_PORT))
            self._cm_sock = s
            log.debug("Dummy UDP socket bound on %s:%d (suppress ICMP for device callback)",
                      CONTROLLER_IP, CMD_CM_UDP_PORT)
        except OSError as exc:
            log.warning("Could not bind socket on %s:%d (%s) -- "
                        "device AppReady callback may trigger ICMP abort",
                        CONTROLLER_IP, CMD_CM_UDP_PORT, exc)
    ...
    def step_ar_connect(self, include_alarm_cr: bool = True) -> bool:
        ...
        # ── IODConnectRes: discriminate error vs success ───────────────────
        #
        # A valid IODConnectRes MUST contain at least one IOCRBlockRes block
        # after the 20-byte NDR prefix.  Minimum valid stub size is ~50 bytes.
        #
        # ERROR formats observed from TeSys Tera:
        #   Pattern A (classic): 01 01 81 xx  — ErrorCode=0x01, Code1=0x81
        #   Pattern B (variant): 00 40 81 xx  — ErrorCode=0x00, Code1=0x81
        #     (returned after partial ghost-AR clear via DCP Set IP)
        # In both patterns byte[2]=0x81 means AlarmCR resource locked.
        #
        # Detection rules (applied in order, first match wins):
        #   1. stub byte[0]==0x01 AND byte[2]!=0 → classic PNIO error
        #   2. stub length < 30 bytes             → too short, reject
        #   3. NDR ActualCount (bytes 16-19 LE)==0 → no blocks, reject
        #   4. block type at byte 20 == 0x0112    → confirmed success
        is_error = False
        err_status = 0

        # Rule 1 — classic PNIO error (ErrorCode=0x01 in BE byte 0)
        if len(stub_body) >= 4 and stub_body[0] == 0x01 and stub_body[2] != 0x00:
            err_status = struct.unpack_from(">I", stub_body, 0)[0]
            is_error = True

        # Rule 2 — stub too short for any valid ConnectRes (NDR prefix alone = 20 B)
        if not is_error and len(stub_body) < 30:
            log.error("    ConnectRes: stub too short (%d bytes) — device rejected connect",
                      len(stub_body))
            log.error("    Full stub: %s", stub_body.hex())
            if len(stub_body) >= 3 and stub_body[2] == 0x81:
                log.error("    GHOST AR LOCK: byte[2]=0x81 in short error stub")
                err_status = struct.unpack_from(">I", stub_body, 0)[0] if len(stub_body) >= 4 else 0
                self._ghost_ar_detected = True
            is_error = True

        # Rule 3 — NDR ActualCount==0: the NDR prefix says no data bytes follow
        if not is_error and len(stub_body) >= 20:
            actual_count = struct.unpack_from("<I", stub_body, 16)[0]
            if actual_count == 0:
                log.error("    ConnectRes: NDR ActualCount=0 — no IOCRBlockRes blocks "
                          "in response (device rejected connect, compact error format)")
                log.error("    Full stub (%d bytes): %s", len(stub_body), stub_body.hex())
                ec1 = stub_body[2] if len(stub_body) >= 3 else 0
                if ec1 == 0x81:
                    log.error("    GHOST AR LOCK: AlarmCR=0x81 in compact error "
                              "(bytes 0-3: %s)", stub_body[:4].hex())
                    err_status = struct.unpack_from(">I", stub_body, 0)[0] if len(stub_body) >= 4 else 0
                    self._ghost_ar_detected = True
                else:
                    log.error("    Device error bytes: %s",
                              stub_body[:4].hex() if len(stub_body) >= 4 else stub_body.hex())
                is_error = True
        ...
        if is_error:
            if err_status != 0:
                ...
                if ec1 == 0x81 and not self._ghost_ar_detected:
                    log.error("    GHOST AR LOCK — the device has an AlarmCR from a "
                              "previous session.")
                    self._ghost_ar_detected = True
                    found = _extract_uuids_from_buf(stub_body)
                    if found:
                        log.error("    Potential ghost AR UUIDs: %s", [str(u) for u in found])
        ...
        # Rule 4 — confirm success: IODConnectRes block at offset 20
        if len(stub_body) >= 22:
            blk_type = struct.unpack_from(">H", stub_body, 20)[0]
            if blk_type == 0x0112:
                log.debug("    IODConnectRes block (0x0112) confirmed at offset 20")
            elif blk_type not in (0x8100, 0x8102):
                log.warning("    ConnectRes: unexpected block type 0x%04x (expected 0x0112)",
                             blk_type)

        iod_len = struct.unpack_from("<I", stub_body, 4)[0] if len(stub_body) >= 8 else len(stub_body) - 20
        log.info("    ConnectRes OK  (%d bytes, NDR_ArgsLen=%d)", len(resp), iod_len)

        # Switch the transport dport to the device's AR session port.
        # Per PROFINET IEC 61158-6-10, after ConnectReq→ConnectRes the device
        # uses an ephemeral port (observed: 49152) for the remaining AR session.
        # All subsequent DCE/RPC packets (Write, PrmEnd, Release) must go there.
        ar_session_port = (getattr(self.xport, "last_resp_sport", 0)
                           or PNIO_AR_SESSION_PORT)
        if self.xport is not None and ar_session_port != self.xport.dport:
            log.info("    Switching transport dport %d → %d (device AR session port)",
                     self.xport.dport, ar_session_port)
            self.xport.dport = ar_session_port

        return True

    def step_iod_write_mrp(self) -> bool:
        """
        IODWriteReq MRP config — opnum 3, seq 1 (confirmed Tanmay pkt#314).

        Two IODWriteReqHeader blocks followed by 140 bytes of MRP domain RecordData.
        This step is mandatory: the TeSys Tera firmware expects it between ConnectRes
        and PrmEnd.  Skipping it causes PrmEnd to be rejected or ignored.
        """
        log.info("-- IODWriteReq MRP config (opnum=%d, seq=%d) --",
                 OP_WRITE, self.seq_num)
        stub = bytearray(_IOD_WRITE_MRP_STUB_TEMPLATE)
        ar_le = _u(self.ar_uuid)   # bytes_le, same encoding used in ARBlockReq
        stub[28:44] = ar_le
        stub[92:108] = ar_le
        resp = self._send(OP_WRITE, bytes(stub), "IODWrite-MRP")
        if resp is None:
            log.warning("    IODWrite-MRP: no response (device may not require MRP write)")
            return True   # non-fatal: continue to PrmEnd
        ptype = resp[1] if len(resp) > 1 else 0xFF
        if ptype == 0x06:
            log.warning("    IODWrite-MRP: DCE/RPC REJECT (ptype=0x06) — skipping MRP write")
            return True   # non-fatal
        log.info("    IODWrite-MRP response received (%d bytes, ptype=0x%02x)", len(resp), ptype)
        return True
    ...
    def step_wait_app_ready_from_device(self, timeout: float = 8.0) -> bool:
        """
        Wait for device-initiated ApplicationReady callback (BlockType=0x0112, opnum=4)
        and respond with BlockType=0x8112.

        In the TeSys Tera PROFINET implementation (confirmed from Tanmay reference pcap),
        the device sends ApplicationReady TO the controller after PrmEnd.  The controller
        MUST respond.  If it does not, the device stays in PARAMETERIZATION state and
        never starts cyclic RT exchange.

        Falls back to controller-initiated AppReady if device callback not received.
        """
        log.info("?? Waiting for device AppReady callback "
                 "(BlockType=0x0112, opnum=4) ?? timeout=%.0f s", timeout)

        # Bind a dummy socket on CMD_CM_UDP_PORT (34962) to suppress Windows
        # ICMP "Port Unreachable" when the device's callback arrives.
        _callback_sock: Optional[socket.socket] = None
        for port in (CMD_CM_UDP_PORT, PNIO_UDP_PORT):
            try:
                s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
                s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                s.bind((CONTROLLER_IP, port))
                _callback_sock = s
                log.debug("Dummy socket on %s:%d (suppress ICMP for device callback)",
                          CONTROLLER_IP, port)
                break
            except OSError:
                pass
        ...
        def _handler(pkt):
            if not pkt.haslayer(Raw):
                return
            raw = bytes(pkt[Raw])
            # Must be DCE/RPC v4 REQUEST (ptype=0x00) from device
            if len(raw) < 80 or raw[0] != 4:
                return
            if raw[1] != PKT_REQUEST:
                return
            opnum = struct.unpack_from("<H", raw, 68)[0]
            if opnum != OP_CONTROL:
                return

            stub = raw[80:]
            # Search for BlockType 0x0112 at stub start, or after 4/20-byte NDR prefix
            block_off = None
            for off in (0, 4, 20):
                if len(stub) >= off + 2:
                    if struct.unpack_from(">H", stub, off)[0] == 0x0112:
                        block_off = off
                        break
            if block_off is None:
                return

            # Verify ControlCommand = AppReady (0x0002)
            b = stub[block_off:]
            # Block: type(2)+len(2)+ver_hi(1)+ver_lo(1) = 6 bytes header,
            #        then body: padding(2)+ARUUID(16)+SessionKey(2)+padding(2)+ControlCmd(2)+props(2)
            cmd_off_in_body = 6 + 2 + 16 + 2 + 2
            if len(b) < cmd_off_in_body + 2:
                return
            ctrl_cmd = struct.unpack_from(">H", b, cmd_off_in_body)[0]
            if ctrl_cmd != CTRL_APP_READY:
                return

            dev_seq      = struct.unpack_from("<I", raw, 64)[0]
            dev_act_uuid = raw[40:56]
            try:
                dev_act = uuid.UUID(bytes_le=dev_act_uuid)
            except Exception:
                dev_act = self.act_uuid

            log.info("    Device AppReady callback received! "
                     "(BlockType=0x0112, ControlCommand=0x%04x, seq=%d)",
                     ctrl_cmd, dev_seq)

            # Build ACK: BlockType=0x8112 (IODControlRes ApplicationReady)
            body  = struct.pack(">H", 0x0000)
            body += _u(self.ar_uuid)
            body += struct.pack(">H", self.session_key)
            body += struct.pack(">H", 0x0000)
            body += struct.pack(">H", CTRL_APP_READY)
            body += struct.pack(">H", 0x0000)
            ack_block = _block(0x8112, body)
            blen = len(ack_block)
            ndr  = struct.pack("<IIIII", blen, blen, blen, 0, blen)
            ack_stub = ndr + ack_block

            # Build as REQUEST then flip to RESPONSE (ptype byte 1)
            ack_pkt = bytearray(build_request(
                seq_num     = dev_seq,
                opnum       = OP_CONTROL,
                obj_uuid    = self.obj_uuid,
                if_uuid     = PNIO_CM_IF_UUID,
            ...
            ack_pkt[1] = PKT_RESPONSE

            try:
                frame = (Ether(dst=TARGET_MAC, src=CONTROLLER_MAC) /
                         IP(src=CONTROLLER_IP, dst=TARGET_IP) /
                         UDP(sport=CMD_CM_UDP_PORT, dport=PNIO_UDP_PORT) /
                         Raw(load=bytes(ack_pkt)))
                sendp(frame, iface=SCAPY_IFACE, verbose=False)
                log.info("    AppReady ACK sent (BlockType=0x8112) -> "
                         "device should now enter DATA_EXCHANGE")
                ack_sent[0] = True
            except Exception as exc:
                log.warning("    AppReady ACK send error: %s", exc)

            result_event.set()

        sniffer = AsyncSniffer(
            iface   = SCAPY_IFACE,
            filter  = f"ether src {TARGET_MAC}",
            prn     = _handler,
            store   = False,
        )
        sniffer.start()
        time.sleep(0.15)   # allow BPF to arm before device sends callback

        received = result_event.wait(timeout)
        sniffer.stop()
        if _callback_sock is not None:
            try:
                _callback_sock.close()
            except Exception:
                pass

        if not received:
            log.warning("    Device AppReady callback NOT received within %.0f s", timeout)
            log.info("    Falling back to controller-initiated ApplicationReady ...")
            return self.step_application_ready()

        log.info("    ApplicationReady handshake complete (device-initiated flow)")
        return True
    ...
    def read_cyclic_data(self, duration_s: float = CYCLIC_DURATION_S,
        ...
        if count == 0:
            ...
            # Additional diagnostic: capture ANY traffic from device (wider filter)
            log.info("  Running 5-second wide capture to check device activity ...")
            all_from_dev: list = []
            def _wide(p):
                if p.haslayer(Ether):
                    all_from_dev.append(p)
            w = AsyncSniffer(iface=SCAPY_IFACE,
                             filter=f"ether src {TARGET_MAC}",
                             prn=_wide, store=False, timeout=5.0)
            w.start()
            w.join()
            if all_from_dev:
                etypes: dict = {}
                for p in all_from_dev:
                    try:
                        et = int(p[Ether].type)
                        etypes[et] = etypes.get(et, 0) + 1
                    except Exception:
                        pass
                log.info("  Wide capture: %d frames from device. EtherTypes: %s",
                         len(all_from_dev),
                         ", ".join(f"0x{k:04x}({v})" for k, v in
                                   sorted(etypes.items(), key=lambda x: -x[1])[:8]))
                # Show details of first 3 frames
                for p in all_from_dev[:3]:
                    try:
                        rb = bytes(p[Raw].load)[:16] if p.haslayer(Raw) else b""
                        log.debug("    frame: type=0x%04x len=%d raw=%s",
                                  int(p[Ether].type), len(bytes(p)), rb.hex())
                    except Exception:
                        pass
            else:
                log.warning("  Wide capture: NO frames at all from device in 5 s -> "
                            "device may be offline or NIC not in promiscuous mode")
    ...
    def run(self):
            ...
            if not dcp_ok:
                ...
                if arp_ok:
                    ...
                    if acked and ip_result == 0x00:
                        log.info("DCP Set IP accepted. Waiting 3 s "
                                 "(note: this may not clear NVM ghost AR) ...")
                        time.sleep(3.0)
                    else:
                        if acked:
        ...
        # No-alarm variants bypass ghost AR lock (error 0x81) without power cycle.
        profiles = [
        ...
        for attempt, (pname, obj_uuid, subslot, sess_key, wprof, alarm_cr) in enumerate(profiles, start=1):
            ...
            if self._ghost_ar_detected:
                ...
                def _retry_connect_all_profiles(label: str, redcp_set_ip: bool = False) -> bool:
                    ...
                    log.info("Retrying all 6 AR Connect profiles after %s ...", label)
                    retry_profiles = [
                        ("cmd-with-alarm",    PNIO_CMD_OBJ_UUID, 0x0001, 0x0002, COMMANDER_WIRE_PROFILE,         True),
                        ("cmd-no-alarm",      PNIO_CMD_OBJ_UUID, 0x0001, 0x0002, COMMANDER_WIRE_PROFILE_NO_ALARM, False),
                        ("cmd-with-alarm-s1", PNIO_CMD_OBJ_UUID, 0x0001, 0x0001, COMMANDER_WIRE_PROFILE,         True),
                ...
                # ── STEP -1: Spoofed Siemens PLC Release (NVM ghost AR) ──────────────
                # The NVM ghost AR was created by the Siemens PLC demo session.
                # None of the DCP methods can clear it.  The ONLY software fix is to
                # impersonate the original PLC MAC/session and send a proper Release.
                _recovery_ok = False
                log.info("Step -1: Spoofed Siemens PLC Release (clear NVM ghost AR) ...")
                if pnio_spoof_release(timeout=5.0):
                    log.info("  NVM ghost AR cleared via spoof-release! Waiting 3 s ...")
                    time.sleep(3.0)
                    if _retry_connect_all_profiles("spoof-release"):
                        _recovery_ok = True
                    else:
                        log.warning("  Still blocked after spoof-release. Trying station-restart ...")

                # ── STEP 0: Station-restart with our OWN MAC (most reliable) ──
                # The ghost AR was almost certainly created by THIS controller
                # (CONTROLLER_MAC) in a previous crashed session.  Per §4.4.1.6
                # the device MUST abort the old AR when the same MAC reconnects.
                # NOTE: _retry_connect_all_profiles MUST be defined above this call.
                if not _recovery_ok:
                    log.info("Step 0: Auto station-restart with own MAC (§4.4.1.6) ...")
                    if pnio_spoof_connect(timeout=8.0):
                        log.info("  Station-restart succeeded! Waiting 3 s then re-connecting ...")
                        time.sleep(3.0)
                        if _retry_connect_all_profiles("station-restart"):
                            _recovery_ok = True   # connected — skip DCP attempts below
                        else:
                            log.error("Connect still failing after station-restart.")
                            log.error("Run: python tesys_pn_v10.py --wait-hello  (after 60s power cycle)")
                            return

                # --- Attempt 1-4: DCP-based ghost AR clearing (if STEP 0 not needed) ---
                if not _recovery_ok:
                ...
                log.error("AR Connect failed but no GHOST AR detected.")
                log.error("Power-cycle TeSys Tera, wait 10 s, then retry.")
                return

        # ── CRITICAL: wrap EVERYTHING after Connect in try/finally ──────────────
        # Any exit path (error, Ctrl+C, exception) MUST send AR Release.
        # Without this every failed run creates a new ghost AR on the device.
        stop_tx   = threading.Event()
        tx_thr    = None
        _ar_live  = True   # AR is established; Release is our responsibility

        try:
            # ?? FIX: Start cyclic TX IMMEDIATELY after ConnectRes ??????????????
            # Root cause of "0 valid input frames": the device watchdog is
            # WDF × SendClock × ReductionRatio = 3 × 128 × 31.25 μs = 96 ms.
            ...
            # (within ~10 ms of ConnectRes at pkt#310).
            tx_thr = threading.Thread(target=self._tx_loop, args=(stop_tx,), daemon=True)
            tx_thr.start()
            ...
            # seq=1  IODWriteReq MRP config (mandatory per Tanmay reference pcap pkt#314)
            if not self.step_iod_write_mrp():
                log.error("IODWrite-MRP failed — sending AR Release to avoid ghost AR")
                return   # finally block will release the AR

            # seq=2  PrmEnd
            if not self.step_prm_end():
                log.error("PrmEnd failed — sending AR Release to avoid ghost AR")
                return   # finally block will release the AR
            time.sleep(0.05)

            # seq=3  ApplicationReady
            # TeSys Tera uses the device-initiated AppReady model (confirmed Tanmay pcap):
            # device sends BlockType=0x0112 to controller; controller ACKs with 0x8112.
            if not self.step_wait_app_ready_from_device(timeout=8.0):
                log.error("ApplicationReady failed — sending AR Release to avoid ghost AR")
                return   # finally block will release the AR
            time.sleep(0.05)

            # seq=4  Acyclic: read I&M0 identity
            ...
            # Cyclic RX exchange (TX is already running)
            self.read_cyclic_data(stop_tx=stop_tx)

        except KeyboardInterrupt:
            log.info("Ctrl+C — stopping cleanly (AR Release will be sent) ...")
        except Exception as exc:
            log.error("Unexpected error during IO exchange: %s", exc, exc_info=True)
        finally:
            # Always stop TX thread first, then release the AR.
            stop_tx.set()
            if tx_thr is not None:
                tx_thr.join(timeout=2.0)
            if _ar_live:
                log.info("Sending AR Release (cleanup) ...")
...
""",
    ...
    parser.add_argument("--spoof-connect",   action="store_true",
                        help="Station-restart: send fresh ConnectReq from OUR OWN MAC first, "
                             "then from the Tanmay demo PLC MAC (60:7d:09:5b:24:b6). "
                             "Per PROFINET §4.4.1.6 the device MUST abort the ghost AR when "
                             "the same CMInitiatorMAC reconnects with a new ARUUID.")
    ...
    if args.spoof_connect:
        ...
        if ok:
            ...
            log.error("Next option: --dcp-reset-comm, then power-cycle 60+ s, or SoMove USB")
            return
