...
print("\n" + "="*60)
print("PHASE 14: The Absolute Truth")
print("="*60)
...
def cyclic_tx_loop(stop_event):
    ...
    while not stop_event.is_set():
        ...
        # Flawless VLAN-Tagged Priority 6 Frame
...
def check_rpc_response(resp, step_name):
    ...
    if ptype == 0x02: return True, "Accepted"
        ...
        if code == 0x1c01000b or code == 0x1c010006:
            return False, f"REJECTED. Device has a Ghost AR Lock. PLEASE POWER CYCLE THE RELAY."
        return False, f"REJECTED. Code: {hex(code)}"
...
try:
    ...
    print("3. Sending ApplicationReady...")
    ...
    for frame in captured:
        ...
        offset = 12
        if raw_bytes[12:14] == b"\x81\x00": offset += 4 # Skip VLAN tag
    ...
    if valid_frames == 0:
        print("[FAIL] Relay unlocked, but no cyclic data captured. Check Wireshark!")
