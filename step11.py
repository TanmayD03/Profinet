...
print("\n" + "="*60)
print("PHASE 11: The True Master (Dynamic MAC Object UUID)")
print("="*60)
...
def build_rpc_header(activity_uuid, seq_num, opnum, payload_len):
    ...
    # CRITICAL FIX: The Object UUID MUST physically contain the Target MAC Address!
    target_mac_clean = TARGET_MAC.replace(":", "")
    obj_uuid_str = f"dea00000-6c97-11d1-8271-{target_mac_clean}"
    
    hdr += uuid.UUID(obj_uuid_str).bytes_le                           # Device Specific Object UUID
...
def check_rpc_response(resp, step_name):
    ...
    if ptype == 0x02: return True, "Accepted"
        ...
        reasons = {
            0x1c010003: "nca_unk_if (Unknown Interface / MAC Mismatch)",
            0x1c010006: "nca_wrong_boot_time (Ghost AR Lock)",
            0x1c01000b: "nca_wrong_boot_time (Ghost AR Lock)"
        }
...
try:
    ...
    print(f"1. Sending AR Connect using MAC {TARGET_MAC}...")
    ...
    rel_hdr = build_rpc_header(act_uuid, 3, 1, 0) 
