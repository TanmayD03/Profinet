...
print("\n" + "="*60)
print("PHASE 7: The Bulletproof State Machine (Fixed!)")
print("="*60)
...
def check_rpc_response(resp, step_name):
    ...
    elif ptype == 0x06: # CRITICAL FIX: 0x06 is the true DCE/RPC Reject code!
        status_hex = resp[32:36].hex() if len(resp) >= 36 else "Unknown"
        return False, f"REJECTED (Hex Status: {status_hex}. Likely nca_wrong_boot_time / Ghost AR lock)"
    elif ptype == 0x03: return False, "FAULT (Device crashed during parse)"
...
try:
    ...
    if valid_frames == 0:
        print("[FAIL] Handshake perfect, but still no data. Check Windows Firewall or Realtek VLAN settings.")
