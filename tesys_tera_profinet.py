...
import socket, struct, sys, threading, time, uuid, csv, argparse
...
def _rpc_header(pkt_type: int, op: int, call_id: int,
    ...
    hdr += struct.pack("<I I I H H H H H B B", 0, 1, call_id, op, 0xFFFF, 0xFFFF, alloc, 0, 0, 0)
    return hdr
...
class Controller:
    ...
    # ── Motor shortcuts ───────────────────────────────────────────────
    def cmd_run_fwd(self) -> bool:
        self.write_many({"Cmd_Stop":False,"Cmd_Run_Forward":True,"Cmd_Run_Reverse":False})
        return True
    def cmd_run_rev(self) -> bool:
        self.write_many({"Cmd_Stop":False,"Cmd_Run_Forward":False,"Cmd_Run_Reverse":True})
        return True
    def cmd_stop(self) -> bool:
        self.write_many({"Cmd_Stop":True,"Cmd_Run_Forward":False,"Cmd_Run_Reverse":False})
        return True
    def cmd_reset(self) -> bool:
        self.write("Cmd_Fault_Reset",True); time.sleep(0.2); self.write("Cmd_Fault_Reset",False)
        return True
    def cmd_estop(self) -> bool:
        self.write("Cmd_Emergency_Stop",True)
        return True
...
def main():
    parser = argparse.ArgumentParser(description="TeSys Tera PROFINET Commander")
    parser.add_argument("--run-fwd", action="store_true", help="Start motor forward")
    parser.add_argument("--run-rev", action="store_true", help="Start motor reverse")
    parser.add_argument("--stop", action="store_true", help="Stop motor")
    parser.add_argument("--reset", action="store_true", help="Fault reset")
    parser.add_argument("--estop", action="store_true", help="Emergency stop")
    parser.add_argument("--read-all", action="store_true", help="Read and print all inputs")
    ...
    # CLI Execution path
    if any([args.run_fwd, args.run_rev, args.stop, args.reset, args.estop, args.read_all]):
        ctrl = Controller(
            iface=_cfg["iface"],
            station=_cfg["station"],
            mod_id=_cfg["module_id"],
            sc=_cfg["sc"], rr=_cfg["rr"]
        )
        if _cfg.get("device_ip"):
            ctrl.set_device_ip(_cfg["device_ip"])
        try:
            print(f"Connecting to {ctrl._station}...")
            ctrl.connect()
            if not ctrl.wait_valid(5.0):
                print(R("IOPS BAD or Not Connected. Check device."))
                sys.exit(1)
            ...
            if args.run_fwd:
                ...
                print("\nInputs:")
                import pprint
                pprint.pprint(ctrl.read_inputs())

            time.sleep(1) # Give cyclic frames a chance to send
        except Exception as e:
            print(R(f"Error: {e}"))
            sys.exit(1)
        finally:
