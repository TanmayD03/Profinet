...
def build_dcerpc_v4_request(call_id: int,
    ...
    hdr += struct.pack("<I", 0)                                      # server_boot
    hdr += struct.pack("<I", 1)                                      # if_version (Version 1.0)
...
def build_ar_block(ar_uuid: uuid.UUID,
    ...
    body += struct.pack(">I", AR_PROP_PULLMODULE)      # ARProperties
    body += struct.pack(">H", 0x8892)                 # CMInitiatorActivityTimeoutFactor (ignored here, reuse ethertype slot)
    body += struct.pack(">H", PNIO_DCEPORT_UDP)        # CMInitiatorUDPRTPort
...
def build_iocr_block(cr: IOCRConfig, ar_uuid: uuid.UUID) -> bytes:
    ...
    body += struct.pack(">I", 0)                # LT (Ethertype, 0 = use default)
...
class PNIOController:
    ...
    def _open_socket(self):
        ...
        self.sock.bind((self.controller_ip, 0))
        self.sock.settimeout(3)
        log.debug("UDP socket bound to %s:0 (ephemeral) ", self.controller_ip)
    ...
    def run(self):
        ...
        try:
            ...
            #     log.error("BIND failed — aborting")
            #     return
            # time.sleep(0.1)   
...
def main():
    ...
    # if obj_uuid is None:
    #     log.warning("EPM query failed or returned no UUID.")
    #     log.warning("Trying with zero Object UUID (some devices accept this).")
    #     obj_uuid = uuid.UUID(int=0)
