#!/usr/bin/env python3
"""
PROFINET IO Controller — TeSys Tera Motor Management Relay  [v6]
===========================================================
...
def list_interfaces() -> None:
    ...
    Call from a Python shell:
      python -c "from tesys_pn_v6 import list_interfaces; list_interfaces()"
    """
    try:
    ...
    def run(self):
        ...
        if SKIP_DCP_PREFLIGHT:
            ...
            if not dcp_ok:
                ...
                if arp_ok:
                    log.warning(
                        "ARP ping succeeded but DCP Identify failed.\n"
                        "  The device is reachable at Layer 2 but not responding to\n"
                        "  PROFINET DCP multicast — possibly it holds an existing AR.\n"
                        "  Action: power-cycle TeSys Tera to clear the ghost AR, then retry.\n"
                        "  If you want to force ahead anyway, set SKIP_DCP_PREFLIGHT=True."
                    )
                else:
                    log.error(
                        "Both DCP Identify and ARP ping failed.\n"
                        "  The device is NOT reachable at Layer 2.  Check:\n"
                        "  (1) SCAPY_IFACE — correct GUID is marked above\n"
                        "  (2) Ethernet cable / switch port between PC NIC and TeSys Tera\n"
                        "  (3) TeSys Tera is powered on (LEDs should show RUN or FAULT)\n"
                        "  (4) No VLAN or firewall on the switch port\n"
